            ...
            if (map) {
              ...
              if (options.embedSources) {
                map.sourcesContent = map.sources.map((source, idx) => {
                  ...
                  const file = res.id.substring(res.id.length - trimmedSource.length) == trimmedSource ? res.id : trimmedSource;
                  map.sources[idx] = path.relative(options.embedSources.rootPath || '.', file);
